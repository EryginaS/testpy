...
class ListViewTest(TestCase):
    ...
    def test_uses_list_template(self):
        ...
        print(f'/lists/{list_.id}/')
        response = self.client.get(f'/lists/{list_.id}/')
        self.assertTemplateUsed(response, 'list.html')

    def test_displays_only_items_for_that_items(self):
...
class NewItemtest(TestCase):
    ...
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post(f'/lists/add_item/{correct_list.id}/',data={'text': 'A new item for an existing list'})
        self.assertEqual(Item.objects.count(), 1)
        new_item = Item.objects.first()
    ...
    def test_redirects_to_list_view(self):
        ...
        other_list = List.objects.create()
        correct_list = List.objects.create()
        response = self.client.post(f'/lists/add_item/{correct_list.id}/',data={'text': 'A new item for an existing list'})
        print(response)
